...
# corresponding stable release of tensorflow would be Tensorflow 2.1.4
# ==============================================================================
...
TLX_REPO = "https://github.com/TensorCodeGen/tensor-codegen/tree/tensor-codegen-12"
TLX_REPO_BRANCH = "tensor-codegen-12"
...
def init_tlx(work_dir):
    ...
    tlx_dir_root = os.path.join(build_dir, "tensor-codegen")
    tlx_project = os.path.join(tlx_dir_root, "tensor-codegen")

    install_tlx = ["bash", "install.sh"]
    install_tlx_cmd = " ".join(install_tlx)

    exec(install_tlx_cmd, cwd = tlx_project)
...
def extend_tlx_repo(tlx_dir_root):
    ...
    workspace_file = os.path.join(llvm_src,"WORKSPACE")
...
def build_tensorflow(tlx_dir_root):
    ...
    tf_packages = glob.glob("/tmp/tensorflow_pkg/tensorflow-2.1.4*")
