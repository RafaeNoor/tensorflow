...
def index_shuffle(index, seed, max_index):
  ...
  >>> indices = tf.random.experimental.index_shuffle(tf.range(4), seed=[5, 9],
    max_index=3)
  ...
  >>> dataset = dataset.map(
    lambda idx: tf.random.experimental.index_shuffle(idx, [5, 9], 9))
  ...
  >>> print(shuffled_vector)
  <tf.Tensor: shape=(4,), dtype=string, numpy=array(
    [b'e2', b'e0', b'e1', b'e3'], dtype=object)>
  ...
  Args:
    index: An integer scalar tensor or vector with values in [0, `max_index`].
      ...
      shuffled vector.
    seed: A tensor of shape [2] or [n, 2] with dtype uint32. The RNG seed. If
      the rank is unknown during graph building it must be 1 at runtime.
